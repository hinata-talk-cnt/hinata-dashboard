...
(() => {
    ...
    function shiftPeriod(offset) {
        ...
        if (targetType === 'month') {
            ...
            selectPeriod('day', formatDateStr(d));
             ...
             let y = parseInt(targetValue, 10) + offset;
             let pStart, pEnd;
    ...
    function initApp() {
        ...
        sortedYears.forEach(yStr => {
            ...
            addItem('year', '年間', c.tY); 
            if(c.tH1>0) addItem('h1', '上半期', c.tH1); 
            if(c.tH2>0) addItem('h2', '下半期', c.tH2);
        });
    ...
    function updatePeriodNavButtons() {
        ...
        if (currentFilter.type === 'month') {
            ...
            if (nextD > maxDateObj) btnNext.disabled = true;
             ...
             const y = parseInt(currentFilter.value, 10);
    ...
    function renderCalendarWidget() {
        ...
        allLogs.forEach(l => { 
            const p = l.date.split('/').map(Number);
            if(p[0] === calYear && p[1] === (calMonth+1)) { daily[p[2]] = (daily[p[2]]||0) + (parseInt(l.count, 10)||0); }
        });
    ...
    function renderRankingView() {
        ...
        const targets = allMembers.filter(m => {
            if (gen !== 'all' && String(m.gen) !== gen) return false;
    ...
    function renderMemberCatalog() {
        ...
        container.innerHTML = "";
        let gensToShow = [];
        if (selectedGen === 'all') { gensToShow = [...new Set(allMembers.map(m => String(m.gen)))].sort(); } else { gensToShow = [selectedGen]; }

        gensToShow.forEach(g => {
            const targets = allMembers.filter(m => String(m.gen) === g);
    ...
    function renderRecordPage() {
        ...
        if (type === 'wins') {
            ...
            allLogs.forEach(l => { if((parseInt(l.count, 10)||0) > 0) dates.add(l.date); });
            Array.from(dates).forEach(date => {
                let maxInDay = 0; const recs = [];
                allLogs.forEach(l => { if(l.date === date) { const c=parseInt(l.count, 10)||0; if(c>maxInDay) maxInDay=c; recs.push({name:l.name, count:c}); }});
            ...
            dataList = allLogs.map(l => ({ date: l.date, name: l.name, count: parseInt(l.count, 10)||0, color: memberMap[l.name]?.color||'#ccc' })).filter(r => r.count>0).sort((a,b)=>b.count-a.count).slice(0, 30);
            if(dataList.length) maxVal = dataList[0].count;
            let rank = 1;
            dataList.forEach((r, i) => {
                if(i>0 && r.count<dataList[i-1].count) rank=i+1;
            ...
            allLogs.forEach(l => {
                ...
                monthlyData[key] = (monthlyData[key]||0) + (parseInt(l.count, 10)||0);
            });
    ...
    function openMonthlyRankingModal(ym) {
        ...
        ranking.forEach((r, i) => {
            ...
            html += `<tr onclick="openModal('${r.name}', 'month:${ymStr}')">
    ...
    function openDailyRankingModal(dateStr) {
        ...
        const ranking = allLogs
            ...
            .map(l => ({ name: l.name, count: parseInt(l.count, 10) || 0, color: memberMap[l.name]?.color || '#ccc' }))
            .filter(r => r.count > 0)
            .sort((a, b) => b.count - a.count);
        ...
        ranking.forEach((r, i) => {
            ...
            html += `<tr onclick="openModal('${r.name}', 'month:${ymStr}')"><td style="width:50px; text-align:center;"><span class="rank-num ${rc}">${rank}</span></td><td style="width:140px; font-weight:bold;">${r.name}</td><td><div class="bar-wrap"><div class="bar-bg"><div class="bar-fill" style="width:${w}%; background:${r.color}"></div></div><div class="bar-txt">${r.count}</div></div></td></tr>`;
        });
        html += '</tbody></table>';
    ...
    function openModal(name, preferredPeriod = null) {
        ...
        if (preferredPeriod) {
            ...
            if (currentFilter.type === 'day' || currentFilter.type === 'month') {
        ...
        if(type === 'month') {
            ...
            allLogs.forEach(l => {
                if(l.name === name && isDateInPeriod(l.date, filter)) {
                    ...
                    logMap[d] = parseInt(l.count, 10) || 0;
                }
            });
            ...
            const mSum = new Map(); allLogs.forEach(l => { 
                if(l.name === name && isDateInPeriod(l.date, filter)) { const p = l.date.split('/').map(Number); const ym = p[0]+'/'+String(p[1]).padStart(2, '0'); mSum.set(ym, (mSum.get(ym)||0)+(parseInt(l.count, 10)||0)); } 
            });
